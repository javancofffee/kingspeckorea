...
if($is_seometa == 'view') { //게시물
	...
	if($is_seometa == 'content') {
		$seometa['url'] = G5_BBS_URL.'/content.php?co_id='.apms_escape_string(clean_xss_tags($co_id));
	} else {
		$seometa['url'] = G5_BBS_URL.'/page.php?hid='.apms_escape_string(clean_xss_tags($hid));
	}
	$seometa['img']['src'] = $seo_page_img;
} else { //기타
	$seometa['url'] = 'http://'.$_SERVER['HTTP_HOST'].apms_escape_string(clean_xss_tags($_SERVER['REQUEST_URI']));
